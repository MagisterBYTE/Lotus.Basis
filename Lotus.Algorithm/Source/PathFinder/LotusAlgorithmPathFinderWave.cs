	...
	*@{*/
    ...
    public class PathFinderWave : PathFinder
    {
        ...
        public override bool ExpansionWave()
        {
            ...
            var add = true;
            int indicateWall = -2, indicateEmpty = -1;
            _isFoundPath = false;
            _stepWave = 0;
            ...
            for (var y = 0; y < _map.MapHeight; y++)
            {
                for (var x = 0; x < _map.MapWidth; x++)
                {
                    ...
                    {
                        _waveMap[x, y] = indicateWall; // индикатор стены
                    }
                    else
                    {
                        _waveMap[x, y] = indicateEmpty; // индикатор еще не ступали сюда
                    }
                }
            }

            // Если стартовая позиция находится на стене
            if (_waveMap[_start.X, _start.Y] == indicateWall)
            {
                return false;
            }

            // Если финишная позиция находится на стене
            if (_waveMap[_target.X, _target.Y] == indicateWall)
            {
                return false;
            }
            ...
            while (add == true)
            {
                for (var x = 0; x < _map.MapWidth; x++)
                {
                    for (var y = 0; y < _map.MapHeight; y++)
                    {
                        ...
                        if (_waveMap[x, y] == _stepWave)
                        {
                            ...
                            if (x - 1 >= 0 && _waveMap[x - 1, y] == indicateEmpty)
                            {
                                _waveMap[x - 1, y] = _stepWave + 1;
                            }

                            if (x + 1 < _map.MapWidth && _waveMap[x + 1, y] == indicateEmpty)
                            {
                                _waveMap[x + 1, y] = _stepWave + 1;
                            }

                            if (y - 1 >= 0 && _waveMap[x, y - 1] == indicateEmpty)
                            {
                                _waveMap[x, y - 1] = _stepWave + 1;
                            }
                            if (y + 1 < _map.MapHeight && _waveMap[x, y + 1] == indicateEmpty)
                            {
                                _waveMap[x, y + 1] = _stepWave + 1;
                            }
                        }
                ...
                // Решение найдено
                if (_waveMap[_start.X, _start.Y] != indicateEmpty)
                {
                    _isFoundPath = true;
                    add = false;
                }
        ...
        public override void BuildPath()
        {
            ...
            while (found)
            {
                ...
                if (cx - 1 >= 0)
                {
                    currentValue = _waveMap[cx - 1, cy];
                    if (currentValue < minimum && currentValue > 0)
                    {
                        cx--;
