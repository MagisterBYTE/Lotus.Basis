...
namespace Lotus.Maths
{
	...
	*@{*/
    ...
    /// </remarks>
    [Serializable]
    public class BezierQuadratic3D : SplineBase3D
    {
        ...
        public static Vector3Df CalculatePoint(float time, Vector3Df start, Vector3Df handlePoint, Vector3Df end)
        {
            var u = 1 - time;
            var tt = time * time;
            var uu = u * u;
        ...
        /// <param name="end">Конечная точка.</param>
        /// <returns>Позиция точки на кривой Безье.</returns>
        public static Vector3Df CalculatePoint(float time, ref Vector3Df start, ref Vector3Df handlePoint, ref Vector3Df end)
        {
            var u = 1 - time;
            var tt = time * time;
            var uu = u * u;
        ...
        public static Vector3Df CalculateFirstDerivative(float time, Vector3Df start, Vector3Df handlePoint, Vector3Df end)
        {
        ...
        /// <param name="end">Конечная точка.</param>
        /// <returns>Первая производная точки на кривой Безье.</returns>
        public static Vector3Df CalculateFirstDerivative(float time, ref Vector3Df start, ref Vector3Df handlePoint, ref Vector3Df end)
        {
            return (2f * (1f - time) * (handlePoint - start)) + (2f * time * (end - handlePoint));
        }
        #endregion
        ...
        public BezierQuadratic3D(Vector3Df startPoint, Vector3Df endPoint)
        ...
        /// "правой" конечной точки кривой</param>
        /// <returns>Первая производная точки на кривой Безье.</returns>
        public Vector3Df CalculateFirstDerivative(float time)
        {
    ...
    /// </remarks>
    [Serializable]
    public class BezierCubic3D : SplineBase3D
    {
        #region Static methods
        /// <summary>
        ...
        /// <param name="end">Конечная точка.</param>
        /// <returns>Позиция точки на кривой Безье.</returns>
        public static Vector3Df CalculatePoint(float time, in Vector3Df start, in Vector3Df handlePoint1,
        ...
        /// <param name="end">Конечная точка.</param>
        /// <returns>Первая производная точки на кривой Безье.</returns>
        public static Vector3Df CalculateFirstDerivative(float time, in Vector3Df start, in Vector3Df handlePoint1,
            in Vector3Df handlePoint2, in Vector3Df end)
        {
            var u = 1 - time;
        ...
        /// <param name="startPoint">Начальная точка.</param>
        /// <param name="endPoint">Конечная точка.</param>
        public BezierCubic3D(Vector3Df startPoint, Vector3Df endPoint)
                            : base(4)
        {
    ...
    /// </remarks>
    [Serializable]
    public class BezierPath3D : SplineBase3D
    {
        #region Fields
        // Основные параметры
...
#endif
        protected internal bool _isClosed;
#if UNITY_2017_1_OR_NEWER
		[UnityEngine.SerializeField]
#endif
        ...
        public BezierPath3D(params Vector3Df[] pivotPoints)
            : base(pivotPoints)
        {
            _handleModes = new TBezierHandleMode[4];
        ...
        public override Vector3Df CalculatePoint(float time)
        {
            ...
            var point = BezierCubic3D.CalculatePoint(time,
        ...
        /// </remarks>
        /// <param name="pivotPoints">Опорные точки пути.</param>
        public void CreateFromPivotPoints(params Vector3Df[] pivotPoints)
        {
            ...
            for (var i = 0; i < pivotPoints.Length; i++)
            {
                ...
                else
                {
                    ...
                    var q0 = p1 - (tangent * (p1 - p0).Length);
                    var q1 = p1 + (tangent * (p2 - p1).Length);
        ...
        public override void SetControlPoint(int index, Vector3Df point, bool updateSpline = false)
        {
            if (index % 3 == 0)
            {
        ...
        public Vector3Df GetCurveControlPoint(int curveIndex, int pointIndex)
        {
            curveIndex *= 3;
        ...
        public void SetCurveControlPoint(int curveIndex, int pointIndex, Vector3Df position)
        {
            curveIndex *= 3;
